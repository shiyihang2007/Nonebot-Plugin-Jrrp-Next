...
async def draw_img(
    ...
    logger.debug(f"Got img_cover: {img_cover}")
    ...
    logger.debug(f"Step 1 finish, img_cover url: {background_url}")
    ...
    first_round.rounded_rectangle(
        (0, 0, image_width * 2, 150 * 2),
        15 * 2,
        (238, 211, 222, 225),
        width=3 * 2,
    ...
    second_round.rounded_rectangle(
        (0, 0, nameLength * 2, 250 * 2), 20 * 2, (91, 57, 83, 255)
    )
    image.alpha_composite(
        im1.resize((image_width, image_height), Image.Resampling.LANCZOS),
        (0, image_height - 150),
    )
    image.alpha_composite(
        im2.resize((image_width, image_height), Image.Resampling.LANCZOS),
        (50, image_height - 175),
    )
    ...
    format_time_length = ImageFont.truetype(
        str(StaticPath.AlibabaPuHuiTi), 25
    ).getlength(format_time_date)
    draw_time_current = DataText(
        image_width - 10 - format_time_length,
        50,
        25,
        format_time_current,
        StaticPath.AlibabaPuHuiTi,
    )
    draw_time_data = DataText(
        image_width - 45 - format_time_length,
        90,
        25,
        format_time_date,
        StaticPath.AlibabaPuHuiTi,
    )
    draw_time_week = DataText(
        image_width - 45, 130, 25, format_time_week, StaticPath.AlibabaPuHuiTi, "rt"
    )
    ...
    # Step 5
    draw_user_info = DataText(
        60, image_height - 160, 25, "User Info", StaticPath.AlibabaPuHuiTi
    )
    ...
    draw_jrrp = DataText(
        180, image_height - 70, 25, f"今日人品值: {jrrp}", StaticPath.AlibabaPuHuiTi
    )
    draw_nickname = DataText(
        180, image_height - 120, 25, name, StaticPath.AlibabaPuHuiTi
    )
    ...
    image.alpha_composite(
        im3.resize((image_width, image_height), Image.Resampling.LANCZOS),
        (image_width - 300, image_height - 350),
    )
    ...
    draw_hitokoto = DataText(
        image_width - 290, image_height - 340, 20, "今日一言", StaticPath.AlibabaPuHuiTi
    )

    image = draw_text(image, draw_hitokoto)

    CREATED_TEXT_LINES = [
        "Created by Nonebot_Plugin_Jrrp_Next",
        "Style by @MoYoez",
        "Python-Version by @SkyDynamic",
        "  Forked @ShiYihang2007",
    ]
    spacing = 0
    for line in CREATED_TEXT_LINES:
        image = draw_text(
            image,
            DataText(10, 15 + spacing, 18, line, StaticPath.AlibabaPuHuiTi),
            average_color,
            2,
            (255, 255, 255),
        )
